...
function make_emscr_ide(textarea, callbacks) {
  ...
  var file_upload = $('<button/>', {
    title: 'File from Disk',
    html: '&#x2B06;',
  }).appendTo(tb);
  var file_download = $('<button/>', {
    title: 'Download editor contents',
    html: '&#x2B07;',
  }).appendTo(tb);
  var file_open = $('<button/>', {
    title: 'Open File',
    html: '&#x1F4C1;',
  }).appendTo(tb);
  var file_save = $('<button/>', {
    title: 'Save File',
    html: '&#x1F4BE;',
  }).appendTo(tb);
  var file_name = $('<input/>', { title: 'File Name', type: 'text', size: 60 })
    .addClass('code')
    .appendTo(tb);
  //TODO Later: support for deleting files?

  var upload_file = $('<input/>', { type: 'file' });
  var file_up_form = $('<div/>')
    .append($('<form/>').append(upload_file))
    .appendTo(div);
  file_up_form.hide();
  ...
  var cm_resize = function () {
    cm.setSize(fr.clientWidth + 2, fr.clientHeight - 10);
  };
  cm_resize();
  if (window.ResizeObserver) new ResizeObserver(cm_resize).observe(fr);
  else if (window.MutationObserver)
    new MutationObserver(cm_resize).observe(fr, { attributes: true });

  ide.isDirty = function () {
    return cm.getValue() != ide.cleanval;
  };
  ...
  file_upload.click(function () {
    ...
    if (file_up_form.is(':visible')) file_up_form.hide();
    else file_up_form.show();
  });
...
function make_emscr_ide_filebrowser() {
  ...
  fb.isVisible = function () {
    return fb.elem.is(':visible');
  };
  return fb;
}
